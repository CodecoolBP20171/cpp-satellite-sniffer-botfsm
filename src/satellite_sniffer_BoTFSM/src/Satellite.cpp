...
Satellite::Satellite(std::string name, std::string noradId, std::string type,
    ...
    : name(name), noradId(noradId), type(type),
      tle(Satellite::loadTle(name, noradId)), sgp4(tle), _shown(visible),
...
void Satellite::calculate(std::tm &time) {
  DateTime tm(time.tm_year + 1900, time.tm_mon + 1, time.tm_mday, time.tm_hour,
...
void Satellite::toggleShown() { _shown = !_shown; }
