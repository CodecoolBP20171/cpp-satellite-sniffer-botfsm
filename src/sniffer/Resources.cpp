...
void Resources::loadTextures() {
  auto *mapSurface = IMG_Load(mConf.getStringValue("/TextureFiles/MAP").data());
  ...
  auto *atlasSurface = IMG_Load(mConf.getStringValue("/TextureFiles/ATLAS").data());
  ...
  for (int i = 0; i < ARRAY_SIZE; i++) {
    ...
    atlasCoords.emplace(std::make_pair<std::string_view, std::array<Graphics::texture_vertex, 4>>(
        ...
        {Graphics::texture_vertex{0.f, 0.f, xMin, yMax},
         Graphics::texture_vertex{0.f, 0.f, xMax, yMax},
         Graphics::texture_vertex{0.f, 0.f, xMin, yMin},
         Graphics::texture_vertex{0.f, 0.f, xMax, yMin}}));
  }
}
...
const std::array<Graphics::texture_vertex, 4> &Resources::getIconTextureVertices(const std::string_view &iconName) {
