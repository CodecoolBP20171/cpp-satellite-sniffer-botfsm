...
bool Menu::isClicked(const SDL_MouseButtonEvent e, PState &state) { return false; }
...
void Menu::render() {
  ...
  if (ImGui::Begin("menu##menubar",
    ...
    if (ImGui::Button("SATELLITES##menu_sats",
                      ImVec2(Config::getIntOption("Dimensions", "MENU_BUTTON_WIDTH"),
                             Config::getIntOption("Dimensions", "MENU_BUTTON_HEIGHT")))) {
      state = PState::PAUSED;
    }
    ImGui::SameLine(ImGui::GetWindowContentRegionMax().x - (Config::getIntOption("Dimensions", "MENU_BUTTON_WIDTH")) -
